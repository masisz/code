...
bool Game::Initialize()
{
    ...
    // Paddle2
    mPaddlePos2.x = 1024.0f - thickness - 10 - 15;
...
void Game::UpdateGame()
{
    ...
    if (
        ...
        diff <= paddleH / 2.0f &&
        // We are in the correct x-position  mPaddlePos + thickness
        ...
        mBallVel.x < 0.0f)
    {
        mBallVel.x *= -1.0f;
            ...
            mBallPos.x <= mPaddlePos2.x && mBallPos.x >= mPaddlePos2.x - 15.0f &&
    ...
    for (Ball& ball : mBalls)
    {
            ...
            ball.vel.x < 0.0f)
        {
            ball.vel.x *= -1.0f;
                ...
                diff2 <= paddleH / 2.0f &&
                // We are in the correct x-position
                ball.pos.x <= mPaddlePos2.x && ball.pos.x >= mPaddlePos2.x - 15.0f &&
                // The ball is moving to the right
                ball.vel.x > 0.0f)
        {
            ball.vel.x *= -1.0f;
...
void Game::GenerateOutput()
{
    ...
    // Multi Balls
    for (Ball value : mBalls)
    {
        SDL_Rect mball {
            static_cast<int>(value.pos.x - thickness/2),
            static_cast<int>(value.pos.y - thickness/2),
            thickness,
            thickness
        };
